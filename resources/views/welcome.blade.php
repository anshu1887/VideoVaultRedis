    ...
    <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow">
        <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
            <div
                ...
                <h1 class="mb-1 text-2xl font-semibold">Welcome to {{ config('app.name', 'VideoVault') }}</h1>
                <p class="mb-4 text-gray-600 dark:text-gray-300">Discover, upload, and share amazing videos. Browse trending
                    content below or upload your own.</p>

                <div class="flex gap-3 mb-6">
                    <a href="{{ route('videos.index') }}"
                        class="inline-flex items-center px-4 py-2 bg-black text-white rounded-md shadow">
                        <x-icon-play class="mr-2 w-4 h-4"/>Browse Videos
                    </a>
                    <a href="{{ route('videos.create') }}"
                        class="inline-flex items-center px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-md">
                        <x-icon-upload class="mr-2 w-4 h-4"/>Upload
                    </a>
                </div>

                <section>
                    <h2 class="text-lg font-medium mb-3">Trending Now</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($trending ?? [] as $item)
                            @php($v = $item['video'] ?? null)
                            @if ($v)
                                <a href="{{ route('videos.watch', $v->id) }}"
                                    class="block bg-white dark:bg-[#1a1a1a] rounded-lg overflow-hidden shadow hover:shadow-lg transition">
                                    <div class="aspect-video bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                                        <img src="{{ asset('images/placeholder1.svg') }}" alt="thumbnail"
                                            class="w-full h-full object-cover">
                                    </div>
                                    <div class="p-3">
                                        <h3 class="text-sm font-semibold truncate">{{ $v->title }}</h3>
                                        <p class="text-xs text-gray-500 mt-1">{{ number_format($item['views'] ?? 0) }} views
                                        </p>
                                    </div>
                                </a>
                            @endif
            ...
            <div
                class="bg-gradient-to-br from-pink-50 to-white dark:from-black dark:to-gray-900 relative lg:-ml-px -mb-px lg:mb-0 rounded-t-lg lg:rounded-t-none lg:rounded-r-lg aspect-[335/376] lg:aspect-auto w-full lg:w-[438px] shrink-0 overflow-hidden">
